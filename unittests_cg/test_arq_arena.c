...
void test_arq_arena_init(void) {
        for (uint32_t offset = 0; offset < 73; offset++) {
                ...
                EXPECT_EQ_UINT((uint32_t)size_of_header, (uint32_t)8);
                EXPECT_EQ_UINT(arena->SIZE, sizeof(array) - offset - padding(offset) - size_of_header);
                EXPECT_EQ_UINT(arena->size, (uint32_t)0);
        }
}
void test_arq_arena_malloc(void) {
        {
                ...
                uint32_t const size_of_header = offsetof(Arq_Arena, at);
                EXPECT_EQ_UINT(size_of_header, (uint32_t)8);
                for (uint32_t i = 1; i < 10; i++) {
                        (void)arq_arena_malloc(arena, 5);
                        EXPECT_EQ_UINT(arena->SIZE, (uint32_t)100 - size_of_header);
                        EXPECT_EQ_UINT(arena->size, (uint32_t)(i * ARQ_ARENA_SIZE_OF_PADDING));
                }
        }
        {
                for (uint32_t n = 1; n < 73; n++) {
                        ...
                        EXPECT_EQ_UINT(arena->SIZE, (uint32_t)100 - size_of_header);
                        EXPECT_EQ_UINT(arena->size, (uint32_t)(ARQ_ARENA_SIZE_OF_PADDING * ((n + ARQ_ARENA_SIZE_OF_PADDING - 1) / ARQ_ARENA_SIZE_OF_PADDING)));
                }
        }
        {
                char array[100] = {0};
                ...
                uint32_t const size_of_header = offsetof(Arq_Arena, at);
                EXPECT_EQ_UINT(size_of_header, (uint32_t)8);
                EXPECT_EQ_UINT(arena->SIZE, (uint32_t)sizeof(array) - size_of_header);
                (void)arq_arena_malloc(arena, arena->SIZE);
                EXPECT_EQ_UINT(arena->size, arena->SIZE);
                EXPECT_EQ_UINT(arena->size, (uint32_t)92);
        }
}
...
void test_arq_arena_malloc_rest(void) {
        ...
        EXPECT_EQ_UINT(size_of_header, (uint32_t)8);
        {
                ...
                EXPECT_EQ_UINT(NUM_OF_TOKEN, (uint32_t)92);
                EXPECT_EQ_UINT(NUM_OF_TOKEN, (uint32_t)sizeof(buffer) - size_of_header);
        }
}
